import { parseArgs, type CliOptions } from "./parser";
...
async function printList(options: CliOptions, logger: Logger): Promise<void> {
  const selectedFeeds = selectFeeds(options.category === "disclosure" ? "dart" : options.category);
  const errors: string[] = [];
  const allItems: NewsItem[] = [];

  for (const feed of selectedFeeds) {
    try {
      const items = await fetchFeed(feed, { timeoutMs: options.timeoutMs });
      allItems.push(...items);
      await saveHistoryEntry({
        timestamp: new Date().toISOString(),
        feedKey: feed.key, feedLabel: feed.label, url: feed.url,
        status: "success", itemCount: items.length,
      }).catch(() => {});
    } catch (error) {
...
async function printSearch(args: string[], options: CliOptions, logger: Logger): Promise<void> {
  const feed = createSearchFeed({
    query: args.join(" "), site: options.site, phrase: options.phrase,
  ...
  try {
    ...
    await saveHistoryEntry({
      timestamp: new Date().toISOString(), feedKey: feed.key, feedLabel: feed.label, url: feed.url,
      status: "success", itemCount: allItems.length,
    }).catch(() => {});
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
    await saveHistoryEntry({
...
async function printOutput(
  items: NewsItem[], errors: string[], options: CliOptions, logger: Logger,
): Promise<void> {
...
async function printDetail(idOrUrl: string | undefined, options: CliOptions, logger: Logger): Promise<void> {
  ...
  const item = cache.items.find((c) => c.id === idOrUrl || c.link === idOrUrl || c.guid === idOrUrl);
  ...
  if (options.json) { logger.info(JSON.stringify(item, null, 2)); return; }
...
async function printHistory(options: CliOptions, logger: Logger): Promise<void> {
  ...
  if (options.json) { logger.info(JSON.stringify(sorted, null, 2)); return; }
  if (sorted.length === 0) { logger.info("No API call history found."); return; }
...
async function runSelfUpgrade(options: CliOptions, logger: Logger): Promise<void> {
  const result = await selfUpgrade({
    version: options.version, installDir: options.installDir,
    skillDir: options.skillDir, codexSkillDir: options.codexSkillDir,
