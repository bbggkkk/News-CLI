...
/**
 * Atomic write: write to a temporary file first, then rename into place.
 * This prevents partial/corrupt cache files from appearing if the process
 * is interrupted mid-write (e.g. SIGKILL, OOM, power loss).
 */
...
/**
 * Safely parse cache JSON. If the file is corrupt (e.g. from a previous
 * interrupted write), return null instead of crashing.
 */
function parseCachePayload(raw: string): CachePayload | null {
	try {
		...
		// Corrupt JSON — treat as empty cache
		return null;
	}
}
